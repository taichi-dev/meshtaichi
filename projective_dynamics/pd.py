...
ti.init(arch=ti.cpu if args.cpu else ti.cuda, dynamic_index=True)
...
model = mesh.build(Patcher.mesh2meta(args.model, relations=["ce", "cv", "ev"]))

x = model.verts.x
v = model.verts.v
f = model.verts.f
m = model.verts.m
mul_ans = model.verts.mul_ans
...
b = ti.Vector.field(3, dtype=ti.f32, shape=len(model.verts))
r0 = ti.Vector.field(3, dtype=ti.f32, shape=len(model.verts))
p0 = ti.Vector.field(3, dtype=ti.f32, shape=len(model.verts))
y = ti.Vector.field(3, dtype=ti.f32, shape=len(model.verts))
x0 = ti.Vector.field(3, dtype=ti.f32, shape=len(model.verts))
